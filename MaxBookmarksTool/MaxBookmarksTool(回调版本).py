...
class BookmarkTrack(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # 深色白字 / 浅色黑字
        label_col = QColor(255,255,255,200) if _dark_mode else QColor(0,0,0,200)
...
class TitleBar(QWidget):
    ...
    def _toggle_pin(self):
        ...
        if ov.isFloating():
            ...
            ov.setTitleBarWidget(ov._empty_title)
...
class TimelineOverlay(QtWidgets.QDockWidget):
    ...
    def _register_callbacks(self):
        ...
        # 尝试注册范围回调（Max 2021+ 均可用），失败降级定时器
        try:
            rt.callbacks.addScript(
                rt.Name("animationRangeChange"), _bm_on_anim_range_changed,  # ← 改这里
                id=rt.Name("bm_anim_range"))
            self._use_range_callback = True
            print("[Bookmarks] 动画范围：回调模式 ✅")
        except Exception as e:
            self._use_range_callback = False
            print(f"[Bookmarks] 动画范围：回调注册失败({e})，降级定时器")
            self._timer_range = QTimer(self)
            self._timer_range.timeout.connect(
                lambda: self._track.on_anim_range_changed())
    ...
    def _unregister_callbacks(self):
        ...
        for cb_id in ["bm_anim_range","bm_scene_new","bm_scene_open","bm_scene_reset"]:
